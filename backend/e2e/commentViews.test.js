...
// post_id breaks isolation from db!!
...
describe('/comment/:id endpoit', () => {
    ...
    test('Returns many comments', async() => {
        const user = await db.createUser('user.email@email.com', 'aaaa!1')
        const post = await user.createPost({'title':'title', 'content':'content'})
        const comment1 = await user.createComment({'postId':post.id,'content':'comment'})
        const comment2 = await user.createComment({'postId':post.id,'content':'comment'})
        const comment3 = await user.createComment({'postId':post.id,'content':'comment'})
